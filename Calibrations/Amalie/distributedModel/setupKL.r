...
dtHrus[Povodi=='KL',sum(Area)]

SoilKL <- dtHrus[Povodi=='KL',]

SoilKL[RVK >0, meanRVK :=mean(RVK)]
SoilKL[VVK>0, meanVVK :=mean(VVK)]

mVVK <- SoilKL[id==41,meanVVK]
mRVK <- SoilKL[id==41,meanRVK]

SoilKL[is.na(meanVVK), meanVVK :=mVVK]
SoilKL[is.na(meanRVK), meanRVK :=mRVK]

SoilKL[RVK ==0, RVK :=mRVK]
SoilKL[VVK ==0, VVK :=mVVK]
...
Areas <- SoilKL$Area
IdsHrus <- SoilKL$id
dhrusKL <- initdHruModel(nHrus,Areas,IdsHrus)
...
library(RcppDE)
itermaxW=5
decntr<-DEoptim.control(VTR = 0, strategy = 2, bs = FALSE, NP = 2700,
...
plot(p_OBS,RmKL, pch=19, ylim=range(c(RmKL,simBest)),
     main = paste0("Karlův Luh - kalibrace dHRUM - ",nHrus," HRU jednotek"),xlab="P(Qm)", ylab="Qm [mm/den]")

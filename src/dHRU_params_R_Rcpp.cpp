...
void setParamsToAlldHrus(Rcpp::XPtr<dHRUM> dHRUM_ptr, Rcpp::NumericVector ParsVec, Rcpp::CharacterVector ParsNames) {
  ...
  if((numParsNames!=numParsVals) || (numParsNames>38) || (numParsVals>38)) {
    ...
    Rcpp::stop("\n Those values are different or higher or smaller then required number of all dHRU Par Values =38.\n");
  } else {
 ...
 */
    ...
    for(unsigned id=0;id<numParsNames;id++){
      ...
      case par_HRUtype::INTstMax:
        ParsToLoad.push_back(std::make_pair((numberSel) parsVals[id], par_HRUtype::INTstMax));
        // pars[32] = par_dta;
        // std::cout << "New INTstMax --> loaded\n";
        break;

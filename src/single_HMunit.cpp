...
single_HMunit::single_HMunit(): tstRM(0),
  ...
  ups = Current_sHMu_configuration.size();
  InstStLai = false;
...
//   return ;
// }
void single_HMunit::pdm(){

  numberSel overFL = 0.0, evap = 0.0, c_init = 0.0, overFl1 = 0.0, ppInf = 0.0, c_prop = 0.0;
  numberSel next_soil = 0.0, overFl2 = 0.0;
...
numberSel single_HMunit::collieForestFrac(){
...
void single_HMunit::interceptions(interception_STORtype _intrc_STORAGE){
  ...
  switch(_intrc_STORAGE) {
    ...
    case interception_STORtype::van_Dijk:{
      if(get_dta(tstRM, ts_type::TEMP) < get_par(par_HRUtype::TMEL)) {
        interception_vanDijk_winter();
       } else {
              if(get_dta(tstRM, ts_type::TEMP) < get_par(par_HRUtype::TETR)){
                interception_vanDijk_melt();
                } else {
                  interception_vanDijk_summer();
              }
             }
      break;
      }
...
void single_HMunit::interception_RutterGash_melt(){
  ...
  set_varValue((Dc + Ds + Ddfs +Ddfc), tstRM, ts_type::MELV);
...
//Liquid interception dynamics
  ...
  Trof = DcMelt +  Dc + DsMelt + Ds + Ddfc + Ddfs;
  set_varValue(Trof, tstRM, ts_type::TROF);
  set_varValue((DcMelt+DsMelt+Ddfc+Ddfs), tstRM, ts_type::MELV);


  Pref = get_dta(tstRM,  ts_type::TROF) + (1-get_par(par_HRUtype::CSfrac)) * (get_dta(tstRM, ts_type::RAIN)) + get_dta(tstRM, ts_type::MELT);
  set_varValue(Pref, tstRM, ts_type::PREF);
...
void single_HMunit::interception_vanDijk_summer(){
  ...
  numberSel Dc = 0.0, Ec =0.0, Pref =0.0, Dddf = 0.0;
  ...
  Pref = DcMelt + Dddf + Dc + (1-get_par(par_HRUtype::CSfrac)) * (get_dta(tstRM, ts_type::RAIN)) + get_dta(tstRM, ts_type::MELT);
  set_varValue(Pref, tstRM, ts_type::PREF);
...
void single_HMunit::snowPrec(){
  ...
  if(get_dta(tstRM, ts_type::TEMP) < get_par(par_HRUtype::TETR)) {
    ...
    Snoww = 0.0;
    Rain = get_dta(tstRM, ts_type::PREC);
  }

  set_varValue(Snoww,tstRM,ts_type::SNOW);
  set_varValue(Rain,tstRM,ts_type::RAIN);
...
void single_HMunit::snow_DDF(){
  ...
  if ((prevSnoS + (1 - get_par(par_HRUtype::CSfrac)) * (Snoww) - Snow_melt)<0){
    prevSnoS = 0.0;
...
void single_HMunit::run_HB() {
  ...
  for(tstRM=0; tstRM < Numdta ; tstRM++) {
    ...
    get_InterceptionsSmax(intrc_STORAGE);
    snowPrec();
    snow_Melt(Snow_MDL);
...
void single_HMunit::get_InterceptionsSmax(interception_STORtype _intrc_STORAGE){
